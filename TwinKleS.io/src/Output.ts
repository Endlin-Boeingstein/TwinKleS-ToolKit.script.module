...
namespace TwinKleS.Output {
	...
	/** 是否已启用VTS */
	export let g_enabled_virtual_terminal_sequences = false;

	/**
	 * 设置文本属性
	 * 
	 * 仅当 g_enabled_virtual_terminal_sequences == true 时输出控制序列，以避免在非VT环境下将控制序列输出为文本
	 * 
	 * @param attribute 文本属性
	 */
	export function set_text_attribute(
		attribute: WindowsConsoleVirtualTerminalSequences.TextAttribute,
	): void {
		if (g_enabled_virtual_terminal_sequences) {
			normal(WindowsConsoleVirtualTerminalSequences.text_attribute(attribute));
		}
	}
	...
	const k_classify_text_attribute: Record<ClassifyTextTypeName, WindowsConsoleVirtualTerminalSequences.TextAttribute> = {
		default: {
			background: 'default',
			foreground: 'default',
			underline: false,
	...
	export function set_classify_attribute(
		...
		set_text_attribute(k_classify_text_attribute[type]);
	}
