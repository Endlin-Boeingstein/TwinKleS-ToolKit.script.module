/** PvZ2文本表版本转换 */
namespace TwinKleS.Support.PvZ2LawnStringText {

	// ------------------------------------------------

	/** 文本版本 */
	export enum Version {
		text = 'text',           // 文本形式，早期版本（6.?及之前）
		json_map = 'json_map',   // JSON 键值对形式，中期版本
		json_list = 'json_list', // JSON 数组形式，当前版本
	};
	...
	export function convert(
		...
		let string_map = {} as Record<string, string>;
		...
		if (source_version === 'auto') {
			try {
				...
				if (source_variant instanceof Array) {
					...
					actual_source_version = Version.json_list;
				} else {
					source_map = source_variant;
					actual_source_version = Version.json_map;
				}
			} catch (e: any) {
				actual_source_version = Version.text;
			}
		} else {
		...
		switch (actual_source_version) {
			case Version.text: {
				let source_text = Core._Cast.from_CharacterListView_to_JS_String(Core._Cast.from_ByteListView_to_CharacterListView(Core.ByteListView.value(source_data)));
				let key_regexp = /^\[.+\]$/gm;
				let value_regexp = /(.|[\n\r])*?(?=[\n\r]*?(\[|$))/gy;
		...
		switch (dest_version) {
			case Version.text: {
				...
				dest_data = Core._Cast.from_moveable_String_to_ByteArray(Core.String.value(dest_text.join('\n'))).release();
				break;
			}
			case Version.json_map: {
				let dest = {
