/** PTX编解码 */
namespace TwinKleS.Support.PopcapTexture {
	...
	export enum BaseFormat {
		...
		rgb_565 = 'rgb_565',
		rgba_5551 = 'rgba_5551',
		rgb_etc1_a_8 = 'rgb_etc1_a_8',
		rgba_pvrtc4 = 'rgba_pvrtc4',
		rgb_pvrtc4_a_8 = 'rgb_pvrtc4_a_8',
	};

	export enum SpecialFormat {
		rgba_4444_block = 'rgba_4444_block',
		rgb_565_block = 'rgb_565_block',
		rgba_5551_block = 'rgba_5551_block',
		rgb_etc1_a_8_index = 'rgb_etc1_a_8_index',
	};

	export type Format = BaseFormat | SpecialFormat;

	// ------------------------------------------------

	const k_base_format: Record<BaseFormat, CoreX.Tool.Image.TextureFormat[]> = {
		rgba_8888: [
			CoreX.Tool.Image.TextureChannel.rgba_8888,
		],
		argb_8888: [
			CoreX.Tool.Image.TextureChannel.argb_8888_l,
		],
		rgba_4444: [
			CoreX.Tool.Image.TextureChannel.rgba_4444_l,
		],
		rgb_565: [
			CoreX.Tool.Image.TextureChannel.rgb_565_l,
		],
		rgba_5551: [
			CoreX.Tool.Image.TextureChannel.rgba_5551_l,
		],
		rgb_etc1_a_8: [
			CoreX.Tool.Image.TextureCompression.rgb_etc1,
			CoreX.Tool.Image.TextureChannel.a_8,
		],
		rgba_pvrtc4: [
			CoreX.Tool.Image.TextureCompression.rgba_pvrtc4,
		],
		rgb_pvrtc4_a_8: [
			CoreX.Tool.Image.TextureCompression.rgb_pvrtc4,
			CoreX.Tool.Image.TextureChannel.a_8,
		],
	};
	...
	export function compute_data_size(
		...
		let data_size = 0n;
		if (format in BaseFormat) {
			data_size = CoreX.Tool.Image.compute_data_size_n(size, k_base_format[format as BaseFormat]);
		} else {
			switch (format) {
				case SpecialFormat.rgba_4444_block:
				case SpecialFormat.rgb_565_block:
				case SpecialFormat.rgba_5551_block: {
					data_size = CoreX.Tool.Image.compute_data_size_n(size, k_base_format[format.slice(0, -6) as BaseFormat]);
					break;
				}
				case SpecialFormat.rgb_etc1_a_8_index: {
					if (option.rgb_etc1_a_8_index === null) {
						throw new MyError(`option is null`);
					}
					data_size = CoreX.Tool.Image.compute_data_size(size, CoreX.Tool.Image.TextureCompression.rgb_etc1) + CoreX.Tool.Expand.PvZ2CHSRSBTextureAlphaIndex.compute_data_size_with_index_list(size, option.rgb_etc1_a_8_index.index.length);
					break;
				}
			}
		}
	...
	export function encode(
		...
		if (format in BaseFormat) {
			...
			switch (format) {
				...
				case SpecialFormat.rgb_565_block:
				case SpecialFormat.rgba_5551_block: {
					Core.Tool.Expand.PvZ1RSBTexture20SeriesLayout.Encode.encode(image, data, Core.Tool.Image.TextureChannel.value(k_base_format[format.slice(0, -6) as BaseFormat][0] as Core.Tool.Image.JS_TextureChannel));
					break;
				}
				case SpecialFormat.rgb_etc1_a_8_index: {
					if (option.rgb_etc1_a_8_index === null) {
					...
					CoreX.Tool.Image.encode(image, data, CoreX.Tool.Image.TextureCompression.rgb_etc1);
					CoreX.Tool.Expand.PvZ2CHSRSBTextureAlphaIndex.encode_with_map(image, data, option.rgb_etc1_a_8_index.index);
					break;
				}
			}
		}
	...
	export function decode(
		...
		if (format in BaseFormat) {
			...
			switch (format) {
				...
				case SpecialFormat.rgb_565_block:
				case SpecialFormat.rgba_5551_block: {
					Core.Tool.Expand.PvZ1RSBTexture20SeriesLayout.Decode.decode(data, image, Core.Tool.Image.TextureChannel.value(k_base_format[format.slice(0, -6) as BaseFormat][0] as Core.Tool.Image.JS_TextureChannel));
					break;
				}
				case SpecialFormat.rgb_etc1_a_8_index: {
					CoreX.Tool.Image.decode(data, image, CoreX.Tool.Image.TextureCompression.rgb_etc1);
					CoreX.Tool.Expand.PvZ2CHSRSBTextureAlphaIndex.decode_with_map(data, image);
					break;
				}
			}
		}
	...
	export function encode_fs(
		...
		if (format === SpecialFormat.rgb_etc1_a_8_index) {
			option.rgb_etc1_a_8_index = {
				index: CoreX.Tool.Expand.PvZ2CHSRSBTextureAlphaIndex.evaluate_index_list(image_view),
			};
		}
