...
namespace TwinKleS.CoreX {
	...
	export namespace FileSystem {
		...
		export function rename(
			...
			return Core.FileSystem.rename(Core.Path.value(parent_directory), Core.String.value(old_name), Core.String.value(new_name));
		}
		...
		export function create_link(
			...
			return Core.FileSystem.create_link(Core.Path.value(target), Core.Path.value(link), Core.Boolean.value(is_directory));
		}
		...
		export function create_hard_link(
			...
			return Core.FileSystem.create_hard_link(Core.Path.value(target), Core.Path.value(link));
		}
	...
	export namespace JSON {
		...
		export function read<ConstraintT extends Core.JSON.JS_Value>(
			...
			let stream = Core.CharacterStreamView.look(Core._Cast.from_ByteListView_to_CharacterListView(Core.ByteListView.value(data)));
		...
		export function write<ConstraintT extends Core.JSON.JS_Value>(
			...
			buffer: Core.CharacterListView | bigint = Core._Cast.from_ByteListView_to_CharacterListView(g_write_buffer.view()),
		): ArrayBuffer {
			let buffer_if = typeof buffer === 'bigint' ? Core.ByteArray.alloc(Core.Size.value(buffer)) : null;
			let buffer_view = buffer instanceof Core.CharacterListView ? buffer : Core._Cast.from_ByteListView_to_CharacterListView(buffer_if!.view());
			...
			return Core._Cast.from_CharacterListView_to_ByteListView(stream.stream_view()).value;
		}
	...
	export namespace XML {
		...
		export function read_fs(
			...
			let data = FileSystem.read_file(file);
			let string = Core._Cast.from_ByteListView_to_CharacterListView(data.view());
		...
		export function write_fs(
			...
			FileSystem.write_file(file, Core._Cast.from_CharacterListView_to_ByteListView(Core._Cast.from_String_to_CharacterListView(string)));
			return;
		}
	...
	export namespace Tool {
			...
			export namespace Encode {
				...
				export namespace Base64 {
					...
					export function encode_fs(
						...
						let ripe_stream = Core.CharacterStreamView.look(Core._Cast.from_ByteListView_to_CharacterListView(ripe_data.view()));
						Core.Tool.Data.Encode.Base64.Encode.encode(raw_stream, ripe_stream);
						FileSystem.write_file(ripe_file, Core._Cast.from_CharacterListView_to_ByteListView(ripe_stream.stream_view()));
						return;
					}
			...
			export namespace Compress {
				...
				export namespace ZLib {
					...
					export function compress_fs(
						...
						Core.Tool.Data.Compress.ZLib.Compress.compress(raw_stream, ripe_stream, Core.Size.value(BigInt(level)));
		...
		export namespace RTON {
			...
			export function encode_fs(
				...
				let json = JSON.read_fs<Core.Tool.RTON.JS_ValidValue>(json_file);
				let rton_stream = Core.ByteStreamView.look(rton_data_buffer_view);
				Core.Tool.RTON.Encode.encode(json, rton_stream, Core.Boolean.value(enable_string_index), Core.Boolean.value(enable_rtid), Core.Boolean.value(is_whole_rton));
			...
			export function decode_fs(
				...
				let json = Core.JSON.Value.default<Core.Tool.RTON.JS_ValidValue>();
				Core.Tool.RTON.Decode.decode(rton_stream, json, Core.Boolean.value(is_whole_rton));
			...
			export function encrypt_fs(
				...
				let plain_data = FileSystem.read_file(plain_file);
				let cipher_size = Core.Tool.RTON.Encrypt.compute_cipher_size(plain_data.size());
				let cipher_data = Core.ByteArray.alloc(cipher_size);
				let plain_stream = Core.ByteStreamView.look(plain_data.view());
				let cipher_stream = Core.ByteStreamView.look(cipher_data.view());
				Core.Tool.RTON.Encrypt.encrypt(plain_stream, cipher_stream, Core.String.value(key));
				FileSystem.write_file(cipher_file, cipher_stream.stream_view());
				return;
			}
			...
			export function decrypt_fs(
				...
				let plain_size = Core.Tool.RTON.Decrypt.compute_plain_size(cipher_data.size());
				let plain_data = Core.ByteArray.alloc(plain_size);
				let cipher_stream = Core.ByteStreamView.look(cipher_data.view());
				let plain_stream = Core.ByteStreamView.look(plain_data.view());
				Core.Tool.RTON.Decrypt.decrypt(cipher_stream, plain_stream, Core.String.value(key));
			...
			export function encode_then_encrypt_fs(
				...
				let cipher_size = Core.Tool.RTON.Encrypt.compute_cipher_size(plain_stream.size());
				let cipher_data = Core.ByteArray.alloc(cipher_size);
				let cipher_stream = Core.ByteStreamView.look(cipher_data.view());
				Core.Tool.RTON.Encrypt.encrypt(plain_stream, cipher_stream, Core.String.value(key));
		...
		export namespace Package {
			...
			export namespace PAK {
				...
				export function pack_fs(
					...
					let manifest = Core.Tool.Package.PAK.Information.Package.json(JSON.read_fs<Core.Tool.Package.PAK.Information.JS_N.Package>(manifest_file));
					Core.Tool.Package.PAK.Pack.pack(package_stream, manifest, Core.Path.value(resource_directory));
				...
				export function unpack_fs(
					...
					let manifest = Core.Tool.Package.PAK.Information.Package.default();
					Core.Tool.Package.PAK.Unpack.unpack(package_stream, manifest, Core.PathOptional.value(resource_directory));
			...
			export namespace DZ {
				...
				export function pack_fs(
					...
					let package_data_buffer_view = package_data_buffer instanceof Core.ByteListView ? package_data_buffer : package_data_buffer_if!.view();
					let package_stream = Core.ByteStreamView.look(package_data_buffer_view);
					let manifest = Core.Tool.Package.DZ.Information.Package.json(JSON.read_fs<Core.Tool.Package.DZ.Information.JS_N.Package>(manifest_file));
				...
				export function unpack_fs(
					...
					let package_data = FileSystem.read_file(package_file);
					let package_stream = Core.ByteStreamView.look(package_data.view());
					let manifest = Core.Tool.Package.DZ.Information.Package.default();
					Core.Tool.Package.DZ.Unpack.unpack(package_stream, manifest, Core.PathOptional.value(resource_directory));
					if (manifest_file !== null) {
						JSON.write_fs(manifest_file, manifest.json);
					}
					return;
			...
			export namespace RSGP {
				...
				export function pack_fs(
					...
					let package_data_buffer_view = package_data_buffer instanceof Core.ByteListView ? package_data_buffer : package_data_buffer_if!.view();
					let package_stream = Core.ByteStreamView.look(package_data_buffer_view);
					let manifest = Core.Tool.Package.RSGP.Information.Package.json(JSON.read_fs<Core.Tool.Package.RSGP.Information.JS_N.Package>(manifest_file));
					Core.Tool.Package.RSGP.Pack.pack(package_stream, manifest, Core.Path.value(resource_directory));
					FileSystem.write_file(package_file, package_stream.stream_view());
					return;
				}
				...
				export function unpack_fs(
					...
					let package_data = FileSystem.read_file(package_file);
					let package_stream = Core.ByteStreamView.look(package_data.view());
					let manifest = Core.Tool.Package.RSGP.Information.Package.default();
					Core.Tool.Package.RSGP.Unpack.unpack(package_stream, manifest, Core.PathOptional.value(resource_directory));
					if (manifest_file !== null) {
						JSON.write_fs(manifest_file, manifest.json);
					}
					return;
			...
			export namespace RSB {
				...
				export function pack_fs(
					...
					let package_data_buffer_view = package_data_buffer instanceof Core.ByteListView ? package_data_buffer : package_data_buffer_if!.view();
					let package_stream = Core.ByteStreamView.look(package_data_buffer_view);
					let manifest = Core.Tool.Package.RSB.Information.Package.json(JSON.read_fs<Core.Tool.Package.RSB.Information.JS_N.Package>(manifest_file));
					Core.Tool.Package.RSB.Pack.pack(package_stream, manifest, Core.Null.default(), Core.Path.value(resource_directory), Core.PathOptional.value(packet_file), Core.PathOptional.value(new_packet_file));
					FileSystem.write_file(package_file, package_stream.stream_view());
					return;
				}
				...
				export function unpack_fs(
					...
					Core.Tool.Package.RSB.Unpack.unpack(package_stream, manifest, Core.Null.default(), Core.PathOptional.value(resource_directory), Core.PathOptional.value(packet_file));
					if (manifest_file !== null) {
						JSON.write_fs(manifest_file, manifest.json);
					}
		...
		export namespace Image {
			...
			export enum TextureChannel {
				...
				rgba_8888 = 'rgba_8888',
				rgb_565_l = 'rgb_565_l',
				rgba_4444_l = 'rgba_4444_l',
				rgba_5551_l = 'rgba_5551_l',
				argb_4444_l = 'argb_4444_l',
				argb_8888_l = 'argb_8888_l',
			}
			...
			export type TextureFormat = TextureChannel | TextureCompression;
			...
			export function get_bpp(
				...
				let result: bigint;
				switch (format) {
					case TextureChannel.a_8: {
						result = 8n;
						break;
					}
					case TextureChannel.rgb_888: {
						result = 24n;
						break;
					}
					case TextureChannel.rgba_8888: {
						result = 32n;
						break;
					}
					case TextureChannel.rgb_565_l: {
						result = 16n;
						break;
					}
					case TextureChannel.rgba_4444_l: {
						result = 16n;
						break;
					}
					case TextureChannel.rgba_5551_l: {
						result = 16n;
						break;
					}
					case TextureChannel.argb_4444_l: {
						result = 16n;
						break;
					}
					case TextureChannel.argb_8888_l: {
						result = 32n;
						break;
					}
					case TextureCompression.rgb_etc1: {
						result = 4n;
						break;
					}
					case TextureCompression.rgb_pvrtc4: {
						result = 4n;
						break;
					}
					case TextureCompression.rgba_pvrtc4: {
						result = 4n;
						break;
					}
				}
		...
		export namespace Expand {
			...
			export namespace PvZ2CHSRSBTextureAlphaIndex {
				...
				export function evaluate_index_list(
					image: Core.Tool.Image.ConstantBitmapView,
				): Core.Tool.Expand.PvZ2CHSRSBTextureAlphaIndex.JS_IndexList {
					let image_size = image.size().value;
					...
					Image.encode(image, image_stream, Image.TextureChannel.a_8);
				...
				export function encode_with_map(
					image: Core.Tool.Image.ConstantBitmapView,
					data: Core.OByteStreamView,
					index: Core.Tool.Expand.PvZ2CHSRSBTextureAlphaIndex.JS_IndexList,
				): void {
					...
					Core.Tool.Expand.PvZ2CHSRSBTextureAlphaIndex.Encode.encode(image, data, index);
					return;
				}
				...
				export function decode_with_map(
					...
					let index: Core.Tool.Expand.PvZ2CHSRSBTextureAlphaIndex.JS_IndexList;
					if (index_count === 0n) {
					...
					Core.Tool.Expand.PvZ2CHSRSBTextureAlphaIndex.Decode.decode(data, image, index);
					return;
				}
			...
			export namespace PvZ2CHSRSBSMFCompress {

				export function compress_fs(
					raw_file: string,
					ripe_file: string,
					level: Data.Compress.ZLib.CompressionLevel = 9,
				): void {
					let raw_data = FileSystem.read_file(raw_file);
					let ripe_size_bound = Core.Tool.Expand.PvZ2CHSRSBSMFCompress.Compress.compute_ripe_size_bound(raw_data.size());
					let ripe_data = Core.ByteArray.alloc(ripe_size_bound);
					let raw_stream = Core.ByteStreamView.look(raw_data.view());
					let ripe_stream = Core.ByteStreamView.look(ripe_data.view());
					Core.Tool.Expand.PvZ2CHSRSBSMFCompress.Compress.compress(raw_stream, ripe_stream, Core.Size.value(BigInt(level)));
					FileSystem.write_file(ripe_file, ripe_stream.stream_view());
					return;
				}
				...
				export function uncompress_fs(
					...
					let ripe_data = FileSystem.read_file(ripe_file);
					let raw_size = Core.Tool.Expand.PvZ2CHSRSBSMFCompress.Uncompress.compute_raw_size(ripe_data.view());
					let raw_data = Core.ByteArray.alloc(raw_size);
					let ripe_stream = Core.ByteStreamView.look(ripe_data.view());
					let raw_stream = Core.ByteStreamView.look(raw_data.view());
					Core.Tool.Expand.PvZ2CHSRSBSMFCompress.Uncompress.uncompress(ripe_stream, raw_stream);
					FileSystem.write_file(raw_file, raw_stream.stream_view());
					return;
				}
	...
	export namespace Process {
		...
		export function system(
			...
			return TwinKleS.Core.Process.system(TwinKleS.Core.String.value(command)).value;
		}
		...
		export function exit(
			...
			return TwinKleS.Core.Process.exit(Core.IntegerS.value(code));
		}

		export function delay(
			duration: bigint,
		): void {
			return TwinKleS.Core.Process.delay(Core.Size.value(duration));
		}
		...
		export function pause(
			...
			return TwinKleS.Core.Process.pause();
		}

		export function input(
		): string {
			return TwinKleS.Core.Process.input().value;
		}

		export function output(
			text: string,
		): void {
			return TwinKleS.Core.Process.output(TwinKleS.Core.String.value(text));
		}
		...
		export namespace Windows {
			...
			export function get_standard_console_mode(
				handle_name: Core.Process.Windows.JS_StandardHandleName,
			): number {
				return Number(TwinKleS.Core.Process.Windows.get_standard_console_mode(Core.String.value(handle_name)).value);
			}
			...
			export function set_standard_console_mode(
				...
				return TwinKleS.Core.Process.Windows.set_standard_console_mode(Core.String.value(handle_name), Core.IntegerU32.value(BigInt(mode)));
			}
			...
			export function select_file_by_dialog(
				...
				return TwinKleS.Core.Process.Windows.select_file_by_dialog(Core.Size.value(BigInt(max_length))).value;
			}
	...
	export namespace _JS {
		...
		export function evaluate_fs(
			...
			return Core._JS.evaluate(Core._Cast.from_ByteListView_to_CharacterListView(script.view()), Core.String.value(script_file));
		}

		export function compile_fs(
			script_file: string,
			byte_code_file: string,
		): void {
			let script = FileSystem.read_file(script_file);
			let byte_code = Core._JS.compile(Core._Cast.from_ByteListView_to_CharacterListView(script.view()), Core.String.value(script_file));
			FileSystem.write_file(byte_code_file, byte_code);
			return;
		}

		export function evaluate_byte_code_fs(
			script_file: string,
		): any {
			let script = FileSystem.read_file(script_file);
			return Core._JS.evaluate_byte_code(script.view());
		}
